...
final class WhoopScoringOrchestrator {
    ...
    private func inferSleepWindow(
        ...
        // and the fallback (first-to-most-recent HR sample) then grows unbounded every
        // refresh since "most recent" is always close to "now". Merge pairs separated
        // by a small gap (skin-contact noise, not real removal) into one continuous
        // interval before picking the longest.
